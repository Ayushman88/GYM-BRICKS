...
>
  ...
  >
    <h4
      class="text-2xl sm:text-3xl md:text-4xl max-w-[1000px] w-full font-medium relative pr-10 after:top-full after:ledt-0 after:w-1/5 after:h-1/5 after:mt-1 after:bg-slate-900"
    >
      <slot />
    </h4>
    ...
    <div class="flex items-center">
      <button class="specialBtnDark ml-auto mr-auto font-semibold"
        ><p>Try free today &rarr;</p></button
      >
    </div>
  </div>
  <div
  ...
  >
    <div
      class="rounded-t-xl h-8 sm:h-10 bg-white opacity-90 px-3 flex items-center gap-2"
    >
      {#each [1, 2, 3] as i}
        <div
          class="rounded-full aspect-square w-2.5 sm:w-3 bg-indigo-400"
        ></div>
      {/each}
    </div>
